...
const char * const BITCOIN_CONF_FILENAME = "firelynxcoin.conf";
const char * const BITCOIN_PID_FILENAME = "firelynxcoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "firelynxcoin" is a composite category enabling all FIRE-related debug output
            if(ptrCategory->count(string("firelynxcoin"))) {
...
#else
    const char* pszModule = "firelynxcoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Mac: ~/Library/Application Support/FIRECore
    // Unix: ~/.firelynxcoinv2
#ifdef WIN32
    // Windows
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty firelynxcoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override firelynxcoin.conf
