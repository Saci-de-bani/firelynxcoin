...
*Setup instructions for a Gitian build of Firelynx Coin using a Debian VM or physical system.*
...
- [Building Firelynx Coin](#building-firelynxcoin-core)
...
Clone the git repositories for Firelynx Coin and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/firelynxcoinpay/firelynxcoin
```
...
in the Firelynx Coin repository under 'Fetch and build inputs' to install sources which require
...
To build Firelynx Coin (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Firelynx Coin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/firelynxcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/firelynxcoinpay/firelynxcoin
...
For example:
```bash
URL=https://github.com/crowning-/firelynxcoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit firelynxcoin=${COMMIT} --url firelynxcoin=${URL} ../firelynxcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit firelynxcoin=${COMMIT} --url firelynxcoin=${URL} ../firelynxcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit firelynxcoin=${COMMIT} --url firelynxcoin=${URL} ../firelynxcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the firelynxcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../firelynxcoin/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/firelynxcoinpay/firelynxcoin-detached-sigs.git

BTCPATH=/some/root/path/firelynxcoin.git
SIGPATH=/some/root/path/firelynxcoin-detached-sigs.git

./bin/gbuild --url firelynxcoin=${BTCPATH},signature=${SIGPATH} ../firelynxcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/firelynxcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/firelynxcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/firelynxcoin-osx-build.assert
```
...
[firelynxcoin/gitian.sigs](https://github.com/firelynxcoinpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
